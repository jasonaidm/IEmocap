...
inter_audio_output = Model(inputs=word_input, outputs=word_attention)
...
inter_text_output = Model(inputs=text_input, outputs=text_attention)
...
#merge = add([text_f_input, audio_f_input])
...
for i in range(1):
    ...
    if acc_t >= text_acc:
        text_acc = acc_t
        train_text_inter = inter_text_output.predict(train_text_data, batch_size=batch_size)
        test_text_inter = inter_text_output.predict(test_text_data, batch_size=batch_size)
...
for i in range(1):
    ...
    if acc_a >= audio_acc:
        audio_acc = acc_a
        train_audio_inter = inter_audio_output.predict_generator(data_generator_output(audio_path, train_audio_data, train_label,
                                                                               len(train_audio_data)),
                                                                steps=len(train_audio_data))
        test_audio_inter = inter_audio_output.predict_generator(data_generator_output(audio_path, test_audio_data, test_label,
                                                                              len(test_audio_data)),
                                                               steps=len(test_audio_data))
        test_audio_weight = inter_audio_weight.predict_generator(data_generator_output(audio_path, test_audio_data, test_label,
                                                                                       len(test_audio_data)),
                                                                 steps=len(test_audio_data))
